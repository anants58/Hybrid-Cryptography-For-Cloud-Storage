...
def encrypt():
	...
	def keyGenerator(pubKey_fname, file, iv):
	    ...
	    h = SHA256.new(str(random.getrandbits(1024)).encode("utf8"))
	...
	def encipher(keyA_fname, keyB_fname, file, password):
	    ...
	    keyCipher = AES.new(str(k)[:32].encode("utf8"), AES.MODE_CFB, iv)
...
if __name__ == '__main__':
    app.run(host=os.getenv('IP', 'localhost'),port=int(os.getenv('PORT', 8089)))